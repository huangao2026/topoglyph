...
**核心原理**：通过加权距离计算两个符号的拓扑相似度，实现跨文明同源性分析。

#### 3. 三层拓扑不变量体系

| 层级 | 核心指标 | 主要作用 |
|------|---------|---------|
| 全局锚点层 | 对称性、宽高比 | 跨文明同源性的基础锚点 |
| 核心不变量层 | 贝蒂数（B₀/B₁） | 拓扑结构的核心描述 |
| 局部指纹层 | 环数、欧拉特征数 | 文化特异性的精细区分 |

**自适应特性**：针对不同语义类型（天体类、自然类、人体类、器物类）自动调整权重配置。
...
| 层级 | 核心特征 | 主要作用 |
|------|---------|---------|
| **第一层：全局锚点层** | 对称性、宽高比 | 跨文明同源性的基础锚点 |
| **第二层：核心不变量层** | 贝蒂数（B₀/B₁）、欧拉示性数 | 拓扑结构的核心描述 |
| **第三层：局部指纹层** | 环数分布、连通分量 | 文化特异性的精细区分 |

**自适应权重机制**：针对不同语义类型（天体类、自然类、人体类、器物类）自动调整各层权重比例。
...
其中：
- ω_i：第i个拓扑特征的权重（自适应调整）
- T_i(a)：符号a的第i个拓扑特征值
- T_i(b)：符号b的第i个拓扑特征值

#### 4. 核心发现

- **对称性是关键判别指标**：跨文明同源性分析的核心依据
- **环数的辅助作用**：区分独立起源与文化传播的参考指标
- **语义类型自适应**：不同语义类型的特征权重配置动态调整
...
**重要说明**：本项目涉及核心技术知识产权保护。

- 📄 [INTELLECTUAL_PROPERTY_PROTECTION.md](INTELLECTUAL_PROPERTY_PROTECTION.md) - 完整的知识产权保护指南
- 🔐 核心权重配置和统计验证数据属于内部机密
- 📚 公开文档仅包含原理性描述和框架说明
